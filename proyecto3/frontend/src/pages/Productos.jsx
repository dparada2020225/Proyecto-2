...
export default function Productos() {
  ...
  const puedeEditar   = puedeMutar(rol, 'productos')
  const puedeEliminarP = puedeEliminar(rol, 'productos')
      ...
      <div className="card" style={{ padding: 0 }}>
        ...
        <div className="table-wrap">
          <table>
            <thead>
              <tr>
                <th>ID</th><th>Nombre</th><th>Precio</th><th>Stock</th><th>Categoría</th><th>Proveedor</th>
                {(puedeEditar || puedeEliminarP) && <th>Acciones</th>}
              </tr>
            </thead>
            <tbody>
              {productos.map(p => (
                <tr key={p.id_producto}>
                  ...
                  {(puedeEditar || puedeEliminarP) && (
                    <td>
                      <div className="td-actions">
                        {puedeEditar    && <button className="btn btn-sm btn-edit"   onClick={() => handleEditar(p)}>Editar</button>}
                        {puedeEliminarP && <button className="btn btn-sm btn-delete" onClick={() => handleEliminar(p.id_producto)}>Eliminar</button>}
                      </div>
                    </td>
                  )}
                </tr>
